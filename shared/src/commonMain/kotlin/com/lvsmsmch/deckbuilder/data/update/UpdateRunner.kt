    ...
    suspend fun runOnce(reason: String, allowMetered: Boolean = true): Boolean {
        ...
        if (cardsDownloadAllowed) {
            runCatching { hsJson.ensureLoaded(locale) }
                .onFailure {
                    AppLog.w(TAG, "ensureLoaded failed: ${it.message}", it)
            ...
            runCatching { hsJson.checkForUpdate(locale) }
                .onSuccess { newBuild ->
                    if (newBuild != null) {
                        applied = true
                        AppLog.i(TAG, "cards updated to build=$newBuild")
                        notifier.emit(UpdateEvent.CardsUpdated(newBuild))
                    }
                }
                .onFailure {
                    AppLog.w(TAG, "checkForUpdate failed: ${it.message}", it)
                    crash.log("HsJson checkForUpdate failed: ${it.message}")
                }
        } else {
            AppLog.i(TAG, "skipping cards check: metered network and mobile download not allowed")
        }
        ...
        runCatching {
            ...
            AppLog.w(TAG, "rotation refresh failed: ${it.message}", it)
        ...
        runCatching {
            ...
            if (status.isOutdated) {
                AppLog.i(TAG, "rotation outdated: unknown=${status.unknownSets.take(5)}…")
            }
        }.onFailure {
            AppLog.w(TAG, "rotation cross-check failed: ${it.message}", it)
        }
