...
fun DeckStatsDialog(deck: Deck, onDismiss: () -> Unit) {
    ...
    AlertDialog(
        ...
        title = { Text(stringResource(Res.string.deck_stats_title), color = DeckBuilderColors.OnSurface) },
        text = {
            Column(modifier = Modifier.fillMaxWidth()) {
                StatRow(stringResource(Res.string.stats_cards), "${deck.cardCount}/${deck.maxCardCount}")
                StatRow(stringResource(Res.string.stats_avg_mana), formatFixed(stats.avgManaCost, 1))
                StatRow(stringResource(Res.string.stats_minions), stats.minionCount.toString())
                StatRow(stringResource(Res.string.stats_spells), stats.spellCount.toString())
                StatRow(stringResource(Res.string.stats_weapons), stats.weaponCount.toString())
                StatRow(stringResource(Res.string.stats_other), stats.otherCount.toString())
                StatRow(stringResource(Res.string.stats_legendary), stats.legendaryCount.toString())
                StatRow(stringResource(Res.string.stats_dust), stats.totalDust.toString())
            }
        },
        confirmButton = {
            TextButton(onClick = onDismiss) {
                Text(stringResource(Res.string.action_close), color = DeckBuilderColors.OnSurface)
            }
        },
    )
}
