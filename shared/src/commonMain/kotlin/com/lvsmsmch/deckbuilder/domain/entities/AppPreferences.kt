...
import com.lvsmsmch.deckbuilder.util.systemCountry
import com.lvsmsmch.deckbuilder.util.systemLanguage
...
object SupportedCardLocales {
    ...
    fun defaultForSystem(
        language: String = systemLanguage(),
        country: String = systemCountry(),
    ): String {
        val exact = "${language}_$country"
        return codes.firstOrNull { it.first.equals(exact, ignoreCase = true) }?.first
            ?: codes.firstOrNull { it.first.substringBefore('_').equals(language, ignoreCase = true) }?.first
            ?: "en_US"
    }
}
